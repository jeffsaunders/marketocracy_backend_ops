<?php
/*
This process runs as a server daemon, controlled by xinetd, listening for connections on ports 52000 - 52099 (API1)
...
/// Membership
	...
	- newFund - Create a new fund
		...
		Example:
		- newFund|jeffsaunders|long|New%20Fund|NEW1|1-1
	...
	- deactivateFund - Deactivate an existing fund
		...
		Example:
		- deactivateFund|jeffsaunders|NEW1|1-3
	...
	- updateName - Change a fund's name
		...
		Example:
		- updateName|jeffsaunders|1-2|JSF|My%20Short%20Fund
	...
	- importPassword - Obtain and encrypt an imported member's password (build auth record)
		...
		Example:
		- importPassword|jeffsaunders|n0thing!|jeff.saunders@marketocracy.com|1
...
/// Fund Data
	...
	- allPositionInfo|jeffsaunders|1-1|JMF or |1 (Active Positions Only)
	- allPositionInfo|jeffsaunders|1-1|JMF|0 (All Positions)
	- tradesForPosition|jeffsaunders|1-1|JMF|AAPL|20140101
	- tradesForFund|jeffsaunders|1-1|JMF|20140101
	- untrade|F8DB1C6D54F0B0C2C0A86F98
	- positionStratification|jeffsaunders|1-1|JMF
	- positionStratification|jeffsaunders|1-1|JMF|1 (Active Positions Only)
	- stylePositionStratification|jeffsaunders|1-1|JMF
	- stylePositionStratification|jeffsaunders|1-1|JMF|1 (Active Positions Only)
	- sectorPositionStratification|jeffsaunders|1-1|JMF
	- sectorPositionStratification|jeffsaunders|1-1|JMF|1 (Active Positions Only)

	Special versions for populating statistical history tables (Add a capital H to the fundID):
	- aggregateStatistics|jeffsaunders|1-1H|JMF|20141231 (get values for last day of the year)
	- alphaBetaStatistics|jeffsaunders|1-1H|JMF|20141231
...
/// Rankings Data
	...
	Examples:
	- quarterlyRanksForFund|jeffsaunders|1-1|JMF
...
// Membership methods
	case "newManager":
		...
		$email		= rawurlencode(trim($aInput[2]));
		$memberID	= trim($aInput[3]);
		$process	= "manageradmin";
		$xmlString	= "<manageradmin><method>".$method."</method><login>".$login."</login><email>".$email."</email><member_ID>".$memberID."</member_ID></manageradmin>";
		break;

	case "newFund":
		$login		= rawurlencode(trim($aInput[1]));
		$type		= trim($aInput[2]);
		$name		= rawurlencode(trim($aInput[3]));
		$symbol		= rawurlencode(trim($aInput[4]));
		$fundID		= trim($aInput[5]);
		$process	= "manageradmin";
		$xmlString	= "<manageradmin><method>".$method."</method><login>".$login."</login><longOrShort>".$type."</longOrShort><name>".$name."</name><fundSymbol>".$symbol."</fundSymbol><fund_ID>".$fundID."</fund_ID></manageradmin>";
		break;

	case "deactivateFund":
		$login		= rawurlencode(trim($aInput[1]));
		$symbol		= rawurlencode(trim($aInput[2]));
		$fundID		= trim($aInput[3]);
		$process	= "manageradmin";
		$xmlString	= "<manageradmin><method>".$method."</method><login>".$login."</login><fundSymbol>".$symbol."</fundSymbol><fund_ID>".$fundID."</fund_ID></manageradmin>";
		break;

	case "updateSymbol":
		$login		= rawurlencode(trim($aInput[1]));
		$fundID		= trim($aInput[2]);
		$oldSymbol	= rawurlencode(trim($aInput[3]));
		$newSymbol	= rawurlencode(trim($aInput[4]));
		$process	= "manageradmin";
	...
	case "updateName":
		...
		$newName	= rawurlencode(trim($aInput[4]));
		$process	= "manageradmin";
	...
	case "managerPassword":
		$login		= rawurlencode(trim($aInput[1]));
		$process	= "manageradmin";
		$xmlString	= "<manageradmin><method>".$method."</method><login>".$login."</login></manageradmin>";
		break;
...
// Trade Data method(s)
	case "positionDetail":
		...
		$start		= trim($aInput[4]);
		$process	= "trade";
		$xmlString	= "<trade><method>".$method."</method><login>".$login."</login><fundSymbol>".$symbol."</fundSymbol><fund_ID>".$fundID."</fund_ID><date>".$start."</date></trade>";
		break;
	...
	case "tradesForPosition":
		...
		$process	= "trade";
		$xmlString	= "<trade><method>".$method."</method><stockSymbol>".$stock."</stockSymbol><login>".$login."</login><fundSymbol>".$symbol."</fundSymbol><startDate>".$start."</startDate><fund_ID>".$fundID."</fund_ID></trade>";
		break;
...
//////Move this to below the end of the switch statement so it is run after each submission-------------------
	...
	case "untrade":
		...
		$process	= "trade";
		$xmlString	= "<trade><method>".$method."</method><ticketKey>".$ticketKey."</ticketKey></trade>";
		break;


// Ranking Data method(s)
