...
/*
								...
								// Get the needed fund data so we can do a livePrice
								$query =	"SELECT f.fund_symbol, m.username
											 FROM members_fund f, members m
											 WHERE f.fund_id = '".$fundID."
											 AND m.member_id = f.member_id
											";
								$rs_fundInfo = mysql_query($query, $linkID); //or die ("ERROR - Query '".$query."' Failed for DB ".$dbName." in MySQL - Process Aborted!");
								$fundInfo = mysql_fetch_assoc($rs_fundInfo)


								// Fire off a livePrice query so the fund has a value
//livePrice|jeffsaunders|1-1|JMF
								$query = "liveprice|".$fundInfo['username']."|".$fundID."|".$fundInfo['fund_symbol']."|".;
								//echo $query."\n";
