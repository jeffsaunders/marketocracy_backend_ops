<?php
/*
This process runs as a server daemon, controlled by xinetd, listening for connections on ports 33000 - 33009
Once connected to a port ranging from 33000 - 33009 simply send it a pipe (|) delimited string of values to pass to an XML request via the Xserve web server.
...
The string must be comma delimited with each element providing the following, in order:
	...
	- create||jeffsaunders|JMF|1-1|sell|GTC|AAPL|50 (minimum for a successful ticket)
...
//die($ticketID);
		$cgiScript	= "ecn.cgi";
		if ($fundKey != ""){
			$xmlString	= "<ecn><method>".$method."</method><ticketID>".$ticketID."</ticketID><fundKey>".$fundKey."</fundKey><stockSymbol>".$stockSymbol."</stockSymbol><buyOrSell>".$action."</buyOrSell><dayOrGTC>".$type."</dayOrGTC><shares>".$shares."</shares></ecn>";
		}else{
			$xmlString	= "<ecn><method>".$method."</method><ticketID>".$ticketID."</ticketID><login>".$login."</login><fundSymbol>".$fundSymbol."</fundSymbol><stockSymbol>".$stockSymbol."</stockSymbol><buyOrSell>".$action."</buyOrSell><dayOrGTC>".$type."</dayOrGTC><shares>".$shares."</shares></ecn>";
		}
...
if (strpos($output, "<H1>Forbidden</H1>")){
	...
	switch ($method){
		...
		case "create":
			...
			$ticketID = $aValues[2]['value']; // returns the passed ticket ID for identification
//echo $ticketKey."\r\n";
