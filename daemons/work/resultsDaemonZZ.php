...
//echo $query."\r\n";
									...
									// Get info about the ticket for the trade record and notification
									$query =	"SELECT *
...
//										$rs_delete = mysql_query($query, $linkID); //or die ("ERROR - Query '".$query."' Failed for DB ".$dbName." in MySQL - Process Aborted!");
										...
										$query = "	UPDATE ".$fund_positions_details_table."
													SET	fund_id	= '".$fundID."',
														...
														first_trade_unix_date = ".strtotime(${$aVal}[13]).",
														last_trade_unix_date = ".strtotime(${$aVal}[14]).",
										";
...
//										if (!$rs_update){
										if (mysql_affected_rows() == 0){
											...
											// Now add the values
											$query .= ") VALUES ('".$fundID."', UNIX_TIMESTAMP(), ".strtotime(${$aVal}[13]).", ".strtotime(${$aVal}[14]).", "; // 13 & 14 are firstTradeTimestamp and lastTradeTimestamp resp.
