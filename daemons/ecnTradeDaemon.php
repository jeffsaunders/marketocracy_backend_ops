...
create||jeffsaunders|JMF|1-1|buy|Day|BRK.B|1|||0
...
//		$type		= trim($aInput[6]);
		$type		= (trim($aInput[6]) == "" ? "GTC" : trim($aInput[6]));

		// Workaround for bizarre BRK.B issue - when defined with the dot this script apparently just dies, so change it to a slash explicitely
		if (trim(strtoupper($aInput[7])) == "BRK.B"){
			$stockSymbol = "BRK/B";
		}else{
...
//die($ticketID);
		...
		if ($fundKey != ""){
			$xmlString	= "<ecn><method>".$method."</method><ticketID>".$ticketID."</ticketID><fundKey>".$fundKey."</fundKey><stockSymbol>".str_replace("/", ".", $stockSymbol)."</stockSymbol><buyOrSell>".$action."</buyOrSell><dayOrGTC>".$type."</dayOrGTC><shares>".$shares."</shares><limit>".$limit."</limit><comment>".$comment."</comment></ecn>";
		}else{
			$xmlString	= "<ecn><method>".$method."</method><ticketID>".$ticketID."</ticketID><login>".$login."</login><fundSymbol>".$fundSymbol."</fundSymbol><stockSymbol>".str_replace("/", ".", $stockSymbol)."</stockSymbol><buyOrSell>".$action."</buyOrSell><dayOrGTC>".$type."</dayOrGTC><shares>".$shares."</shares><limit>".$limit."</limit><comment>".$comment."</comment></ecn>";
		}

		// Update the transaction record with the XML string
		if (!is_numeric($shares)){
			$error = "Non-numeric share quantity";
			update_transaction($transactions_api_table, $linkID, $xmlString, $trans_key, $error);
		...
		$status		= (isset($error) ? "failed" : "pending");
