...
// Modified by: Jeff Saunders - 9/25/18 - Added check for which model the trade is for ("mFolio" indicates old, "FOLIOfn" indicates new) and added an "X" to the fund_id (e.g.24-1X) if it's old (so it won't be processed)
...
// Use for manual processing of specific dates in makeup files (if any)
//$sDate = "2018-08-29";
//$filename = "marketocracy-report-2018-08-29-2018-08-31.csv";
...
//		$fund_symbol = explode("_", explode(': ', $aUpload[1][9])[1])[1];
		...
		$aFolio = explode(':', $row[9]); // Grab just the fund name, to the right of the colon
		$sModel = trim($aFolio[0]);
		$aParts = explode('_', trim($aFolio[1]));  // Blow the whole damn thing up
...
/////////////////////////////// If not found need to create member and/or fund HERE - new fund created on Folio side.
			...
			$fund_id = $ID['fund_id'];
			// If this trade is for the old model then append an X to it's ID
			if ($sModel == "mFOLIO"){
				$fund_id .= "X";
			}
			$folio = $row[9];
...
*/
	// Insert it
	$query = "
		INSERT INTO folio_trade_history (
			member_id,
			fund_id,
			unix_date,
			date,
