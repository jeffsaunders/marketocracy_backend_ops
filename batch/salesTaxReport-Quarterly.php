<?php
/*
The purpose of this script is to generate a periodic (quarterly) Texas sales tax report
...
// Get start date
// Go back 3 months and determine the first second of that month's first day
$start_seed = strtotime('3 months ago');  // 3 months ago from now
$start_quarter = ceil(date('m', $start_seed) / 3);  // Starting quarter number
$start_month = ($start_quarter * 3) - 2;  // Starting month number
$start_year = date('Y', $start_seed);  // Starting year number
$start_timestamp = mktime(0, 0, 0, $start_month, 1, $start_year);  // Timestamp of starting second of quarter
//echo $start_timestamp."\n";

// Get end date
// Determine the first second of this quarter's first day and subtract 1 second to get last second of last quarter
$end_seed = strtotime('now');  // Now
$end_quarter = ceil(date('m', $end_seed) / 3); // This quarter's number
$end_month = ($end_quarter * 3) - 2;  // Starting month of this quarter
$end_year = date('Y', $end_seed);  // This quarter's year
$end_timestamp = mktime(0, 0, 0, $end_month, 1, $end_year) - 1;  // Timestamp of this quarter's first second, minus 1 second (results in timestamp of last last quarter's last second)
//echo $end_timestamp."\n";
...
$subject = 'Sales Tax Report for Quarter '.$start_quarter.', '.$start_year;
