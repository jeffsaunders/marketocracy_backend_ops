...
// Get the masters' funds that we need history for (the ones we are tracking)
$query = "
...
";
try{
	$rs_tracking = $sLink->prepare($query);
...
//die($preparedQuery);
	...
	";
	try {
		$rs_fund = $mLink->prepare($query);
		$aValues = array(
			':fund_id' => $tracking['fund_id']
		);
		$preparedQuery = str_replace(array_keys($aValues), array_values($aValues), $query); //Debug
//die($preparedQuery);
		$rs_fund->execute($aValues);
	}
	catch(PDOException $error){
		// Log any error
		file_put_contents($pdo_log, "-----\rDate: ".date('Y-m-d H:i:s')."\rFile: ". __FILE__ ."\rLine Number: ". __LINE__ ."\rVars:\r".dump_vars(get_defined_vars())."\r", FILE_APPEND);
	}
	$fund = $rs_fund->fetch(PDO::FETCH_ASSOC);

	$aMethods = array("aggregateStatistics","alphaBetaStatistics");
	for ($cnt = 0; $cnt < sizeof($aMethods); $cnt++){
//echo $aMethods[$cnt];
		// Build queries and submit them
		$query = $aMethods[$cnt]."|0|".$member['username']."|".$tracking['fund_id']."H|".$fund['fund_symbol']."|".$date;

//aggregateStatistics|0|jeffsaunders|1-1H|JMF|20150131
//aggregateStatistics|0|adevkota|4272-1H|AMF|20171031
//aggregateStatistics|0|adevkota|4272-5H|AMFSP|20171031
//echo $query."\n\n";
