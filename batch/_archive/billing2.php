...
if ($subs['member_id'] == $member_id){
	...
	// Step through them
	$subIDs = "";
	while($items = $rsItems->fetch(PDO::FETCH_ASSOC)){
		...
		$billAmount = $price;
		$term		= $subs['bill_frequency'];

		// Check for discounts
		$discounts = $subs['discounts'];
		if ($discounts != "" && !is_null($discounts)){

			$aDiscounts = explode("|", $subs['discounts']);
...
//print_r($aDiscount);die();
				...
				$expiration = strtotime('+'.$duration.' days', $subs['start_timestamp']);
