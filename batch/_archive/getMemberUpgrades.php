...
<br><font size="+2">No Upgrades to Report.</font>
	...
	$query = "
		SELECT m.member_id, m.username, m.name_first, m.name_last, m.email, m.phone_day, from_unixtime(ms.start_timestamp, '%m/%d/%Y@%h:%i:%s') as time, p.product_name
		FROM members m, members_subscriptions ms, site_products p
		WHERE ms.start_timestamp >= ".$startTimestamp."
		AND ms.start_timestamp < ".$endTimestamp."
		AND ms.product_id IN (2,3)
...
$email->Subject   = 'Membership Upgrades for '.date("m/j/y", strtotime("yesterday"));
$email->Body      = $message;
