...
// Write header row
$headers = "Exception,Symbol,Message,Name,Market,MarketIDCode,Category,Active Members Hold,From FrontBase,From Xignite,On Static List\r\n";
fwrite($fp, $headers);
...
while($exceptions = mysql_fetch_array($rs_exceptions)){
	...
	// Write the row
	$row = '"'.$exception.'","'.$symbol.'","'.$message.'","'.$name.'","'.$market.'","'.$marketID.'","'.$category.'","'.$holders.'","'.$frontbase.'","'.$xignite.'","'.$static.'"';
	$row .= "\r\n";
	fwrite($fp, $row);
}
