...
// Get all the fund ids so we can do this in chunks (by id)
$query = "
...
//dump_rs($rsFunds); // Display result set - debug

while($fund = $rsFunds->fetch(PDO::FETCH_ASSOC)){

	// Get all the fund trade records for this fund
	$query = "
		SELECT uid, date
		FROM folio_trade_history
...
//dump_rs($rsTrades); // Display result set - debug
//die();
	...
	while($trade = $rsTrades->fetch(PDO::FETCH_ASSOC)){
		...
		$query = "
			UPDATE folio_trade_history
			SET unix_date = :unix_date
			WHERE uid = :uid
		";
		try{
...
/* Find all older billdates - test (scratchpad) query, not part of this utility
	SELECT *
	FROM members_subscriptions
	WHERE active = 1
	AND next_bill_timestamp IS NOT NULL
	AND next_bill_timestamp < 1512424193
	ORDER BY member_id
*/
