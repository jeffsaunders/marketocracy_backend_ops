<?php
/*
The purpose of this script is to build a file of Paid Basic Members for enrollment and grouping within the Forums "class" for assignment within Moodle
...
$fp1 = fopen("/var/www/html/tmp/PaidBasicMembersListForMoodleGrouping.csv", "w");
...
while ($manager = $rsManagers->fetch(PDO::FETCH_ASSOC)){
	...
	// Write the row
	$row = '"'.$firstname.'","'.$lastname.'","'.$username.'","'.$email.'","'.$password.'","Roundtable","1","Basic"'; //Managers,Pro,Plus,Basic,Free
	$row .= "\r\n";
	fwrite($fp1, $row);
