<?php
/*
The purpose of this script is to populate the rank_raw_nav table with the raw NAV data extracted via the getPriceHistoryForFundOnDate.php script.
The getPriceHistoryForFundOnDate.php script could just as easily write it's results directly to the table, but having the intermediary step allows
   for validation and the extract file provides an audit trail (and is available should Ken wish to use it in his internal system).
...
/*  Don't need to look these up any longer since the source of the data is Nova
...
//		);
		$aValues = array(
			':fundkey'			=> NULL,
			':fund_id'			=> ($aNAV['FundID'] != "" ? $aNAV['FundID'] : NULL),
			':managerkey'		=> NULL,
			':member_id'		=> ($aNAV['MemberID'] != "" ? $aNAV['MemberID'] : NULL),
			':as_of_date'		=> date('Ymd', strtotime($aNAV['AsOfDate'])),
			':as_of_timestamp'	=> strtotime($aNAV['AsOfDate']),
			':nav'				=> ($aNAV['NAV'] != "" ? $aNAV['NAV'] : NULL),
