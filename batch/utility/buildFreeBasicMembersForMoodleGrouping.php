<?php
/*
The purpose of this script is to build a file of Free Basic Members for enrollment and grouping within the Forums "class" for assignment within Moodle
...
$fp1 = fopen("/var/www/html/tmp/FreeBasicMembersListForMoodleGrouping.csv", "w");
...
while ($manager = $rsManagers->fetch(PDO::FETCH_ASSOC)){
	...
	$firstname	= addslashes($manager['name_first']);
	$lastname	= addslashes($manager['name_last']);
	$username	= addslashes($manager['username']);
	$email		= addslashes($manager['email']);

	// Make sure they gave a first name, last name, and email address - reject them otherwise
	...
	// Write the row
//	$row = '"'.$firstname.'","'.$lastname.'","'.$username.'","'.$email.'","'.$password.'","Roundtable","1","Free"'; //Managers,Pro,Plus,Basic,Free
	$row = $firstname.','.$lastname.','.$username.','.$email.','.$password.',Roundtable,1,Free'; //Managers,Pro,Plus,Basic,Free
	$row .= "\r\n";
	fwrite($fp1, $row);
