...
$fp1 = fopen("/var/www/html/tmp/ProMembersListForMoodleGrouping.csv", "w");
...
while ($manager = $rsManagers->fetch(PDO::FETCH_ASSOC)){
	...
	// Write the row
	$row = '"'.$firstname.'","'.$lastname.'","'.$username.'","'.$email.'","'.$password.'","Roundtable","1","Pro"'; //Managers,Pro,Plus,Basic,Free
	$row .= "\r\n";
	fwrite($fp1, $row);
