<?php
// This commandline batch script grabs the positionStratification, stylePositionStratification, and sectorPositionStratification for every fund for each member who is currently logged in
...
//print_r($aFunds);//die();
		...
		foreach ($aFunds as $fund_id => $fund_symbol){
			...
			$query = "positionStratification|".$loggedIn['username']."|".$fund_id."|".$fund_symbol;
//echo $query."\n\n";
...
//echo $query."\n\n";
			...
			$cmd = '/var/www/html/scripts/process-legacy-query.sh "'.$port.'" "'.$query.'" > /dev/null &';
			exec($cmd);
			$query = "sectorPositionStratification|".$loggedIn['username']."|".$fund_id."|".$fund_symbol;
//echo $query."\n\n";
