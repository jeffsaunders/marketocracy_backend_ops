...
// Declare an exceptions array to hold all the bills that failed to process
//$aExceptions = array(0 => "X");
//print_r($aExceptions);
$aExceptions = array();
...
while($subs = $rsSubs->fetch(PDO::FETCH_ASSOC)){
	...
	// If the next billing date is far in the past, flag it and skip (added to report below)
	if ($subs['next_bill_timestamp'] < (time() - 7776000)){ // 90 days
echo $subs['member_id']."\n";
		$aExceptions[$subs['member_id']] = "Next Bill Timestamp = ".date('m/d/Y', $subs['next_bill_timestamp']);
//		array_push($aExceptions, $member_id, "Next Bill Timestamp = ".date('Ymd', $subs['next_bill_timestamp']));
//		$aExceptions[$subs['member_id']] = "X";
//		$aExceptions[] = "X";
		continue;
	}
...
//			$rsLogError->execute($aValues);
		...
		$query = "
			INSERT INTO ".$members_notification_table."(
				member_id,
				notification_id,
				notification,
				link,
		...
		try{
			...
			$aValues = array(
				':member_id'	  	=> $member_id,
				':notification_id'	=> "15-002",
				':notification'		=> "Your payment of $".$transaction_amount." was declined. Please check that your default payment method is valid. We will try again tomorrow.",
				':link'				=> "?page=10-00-00-002&account=".$member_id."&tab=payments",
				':flagged'			=> 0,
				':viewed'			=> 0,
				':deleted'			=> 0
			);
...
//			$rsInsert->execute($aValues);
		...
		// PDO BUG!  All strings bound (:var) for a "WHERE IN" get quoted (on the ends, like "1,2,3") thus the comparison for numeric values is made against the entire string, not each comma separated value individually
		// Must use old syntax for proper assignment of the IN value
		$query = "
...
//			$rsUpdate->execute($aValues);
		...
		try{
			...
			$aValues = array(
				':member_id'	  	=> $member_id,
				':notification_id'	=> "15-001",
				':notification'		=> "Your payment of $".$transaction_amount." processed successfully. Thank You.",
				':link'				=> "?page=10-00-00-002&account=".$member_id."&tab=payments&subtab=billing_history",
				':flagged'			=> 0,
				':viewed'			=> 0,
				':deleted'			=> 0
			);
