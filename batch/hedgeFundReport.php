...
// Write header rows
$headers = "Fund_Key,Login Name,Fund Symbol,Current Date,Stock Symbol On Date,Stock CUSIP,Stock Current Shares,Stock Price,Stock Value\r\n";
fwrite($fpPO, $headers);
$headers = "Fund_Key,Current Date,Stock Symbol On Date,Stock CUSIP,Stock Current Shares,Stock Price,Stock Value\r\n";
fwrite($fpPA, $headers);
...
echo $fundkey."\n";
	...
	// Get the positions for this fund on specified date
//	$query = "
...
//print_r($position); die();
	while($position = $rsPositions->fetch(PDO::FETCH_ASSOC)){
		...
		$rowPO = '"'.trim($fundkey).'","'.$position['username'].'","'.$position['fund_symbol'].'","'.$rowDate.'","'.$position['stockSymbol'].'"';
		$rowPA = '"'.trim($fundkey).'","'.$rowDate.'","'.$position['stockSymbol'].'"';
...
//echo $rowTA."\n\n";
	...
	while($trade = $rsTrades->fetch(PDO::FETCH_ASSOC)){
		...
		// Finish building rows
		$rowTO .= ',"'.$trade['buyOrSell'].'","'.$trade['stockSymbol'].'","'.$rowDate.'","'.$trade['sharesFilled'].'","'.$trade['price'].'","'.round($trade['net'], 2).'","'.($trade['createdByCA'] = 0 ? "No" : "Yes").'","'.$trade['stockSymbol'].'","'.$trade['ticketKey'].'"';
		$rowTA .= ',"'.$trade['buyOrSell'].'","'.$trade['stockSymbol'].'","'.$rowDate.'","'.$trade['sharesFilled'].'","'.$trade['price'].'","'.round($trade['net'], 2).'","'.($trade['createdByCA'] = 0 ? "No" : "Yes").'","'.$trade['stockSymbol'].'","'.$trade['ticketKey'].'"';
