...
$aDirectories = array("/api1/","/api2/","/api3/");
...
for ($d = 0; $d < count($aDirectories); $d++){
	...
	for ($p = 0; $p < count($aProcesses); $p++){
		...
		for ($t = 0; $t < count($aTypes); $t++){
			...
			$cmd = "find ".$aDirectories[$d].$aProcesses[$p].$folder" -maxdepth 1 -name \"*".$aTypes[$t]."*\" -type f | wc -l";
			// Execute the command and assign the returned number to a var
			$count = exec($cmd);
