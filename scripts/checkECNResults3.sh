...
#        ((found+=$check))
#        ((found-=1))
####        check=`find /api3/ecn_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
        check=`ls -al /api3/ecn_processing/ecn_output_* | wc -l`
        ((found+=$check))
#        ((found-=2))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep ecnR[e]sultsDaemon3.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/ecnResultsDaemon3.php
#echo
#echo "Start Daemon"
...
#echo $processCheck
		...
		if test "$processCheck" != "" ; then
			...
			#killall php
                        ps -ef | awk '$NF~"ecnResultsDaemon3.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
#echo "Stop Daemon"
