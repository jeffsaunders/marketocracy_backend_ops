...
####        check=`find /api2/fundprice_processing/* -maxdepth 1 -name "*output*" -type f | wc -l`
	check=`ls -al /api2/fundprice_processing/fundprice_output_* | wc -l`
        ((found=$check))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep fundpriceR[e]sultsDaemon2.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/fundpriceResultsDaemon2.php
#echo
#echo "Start Daemon"
...
#echo $processCheck
		...
		if test "$processCheck" != "" ; then
			...
			#killall php
			ps -ef | awk '$NF~"fundpriceResultsDaemon2.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
