...
#        ((found=$check))
#        ((found-=1))
#        check=`find /api3/stockprice_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
        check=`ls -al /api3/stockprice_processing/stockprice_output_* | wc -l`
        ((found+=$check))
#        ((found-=1))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep stockpriceR[e]sultsDaemon3.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/stockpriceResultsDaemon3.php
#echo
#echo "Start Daemon"
...
#echo $processCheck
		...
		if test "$processCheck" != "" ; then
			...
			#killall php
                        ps -ef | awk '$NF~"stockpriceResultsDaemon3.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
#echo "Stop Daemon"
