...
####        check=`find /api/ecn_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
        check=`ls -al /api/ecn_processing/ecn_output_* | wc -l`
        ((found+=$check))
#        ((found-=2))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep ecnR[e]sultsDaemon.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/ecnResultsDaemon.php
#echo
#echo "Start Daemon"
