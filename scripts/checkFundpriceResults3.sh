...
#        check=`find /api2/fundprice_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
####        check=`find /api3/fundprice_processing/* -maxdepth 1 -name "*output*" -type f | wc -l`
        check=`ls -al /api3/fundprice_processing/fundprice_output_* | wc -l`
        ((found=$check))
#        check=`find /api2/fundprice_processing/* -maxdepth 1 -type d -prune -o -name "*output*" -print | grep -c ""`
#        ((found=$check))
#        ((found-=1))
#        check=`find /api/stockprice_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
#        ((found+=$check))
#        ((found-=1))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep fundpriceR[e]sultsDaemon3.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/fundpriceResultsDaemon3.php
#echo
#echo "Start Daemon"

		fi

#                # See if secondary results daemon is running already
#                processCheck=`/bin/ps -ef | grep fundpriceR[e]sultsDaemon22.php`
#
#                # If it's not already running...
#                if test "$processCheck" == "" ; then
#
#                        # Start the results daemon
#                        /usr/bin/php /var/www/html/daemons/fundpriceResultsDaemon22.php
#echo
#echo "Start Daemon"
...
#echo $processCheck
		...
		if test "$processCheck" != "" ; then
			...
			#killall php
			ps -ef | awk '$NF~"fundpriceResultsDaemon3.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
...
#                        # Kill the results daemon
#                        #killall php
#                        ps -ef | awk '$NF~"fundpriceResultsDaemon22.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
#
##echo "Stop Daemon"
#
#		fi
