...
####        check=`find /api/trade_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
        check=`ls -al /api/trade_processing/trade_output_* | wc -l`
        ((found+=$check))
#        ((found-=1))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep tradeR[e]sultsDaemon.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/tradeResultsDaemon.php
#echo
#echo "Start Daemon"
...
#			killall php
                        ps -ef | awk '$NF~"tradeResultsDaemon.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
#echo "Stop Daemon"
