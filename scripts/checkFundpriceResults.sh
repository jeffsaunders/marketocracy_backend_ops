...
####        check=`find /api/fundprice_processing/* -type d -prune -o -name "*output*" -print | grep -c ""`
        check=`ls -al /api/fundprice_processing/fundprice_output_* | wc -l`
        ((found=$check))
#        ((found-=1))
...
#echo "found file(s)"
		...
		processCheck=`/bin/ps -ef | grep fundpriceR[e]sultsDaemon.php`

		# If it's not already running...
		if test "$processCheck" == "" ; then

			# Start the results daemon
			/usr/bin/php /var/www/html/daemons/fundpriceResultsDaemon.php
#echo
#echo "Start Daemon"
...
#echo $processCheck
		...
		if test "$processCheck" != "" ; then
			...
			#killall php
			ps -ef | awk '$NF~"fundpriceResultsDaemon.php" {print $2}'| xargs kill -15 #TERM vs. -9 KILL
